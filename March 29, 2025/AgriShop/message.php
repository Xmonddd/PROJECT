...
// List ng mga user message yan ahhh Function to get conversations (users you've messaged)
function getConversations($conn, $activeUser) {
    $sql = "SELECT DISTINCT u.fullname, u.profile_image 
            FROM users u 
            JOIN messages m ON u.username = m.sender OR u.username = m.receiver 
    ...
    while ($row = $result->fetch_assoc()) {
        $conversations[] = [
            ...
            'profile_image' => isset($row['profile_image']) && !empty($row['profile_image']) ? $row['profile_image'] : 'uploads/default.png'
        ];
    }
