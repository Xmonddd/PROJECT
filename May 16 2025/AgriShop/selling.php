            ...
            <div class="modal-body">
                <form method="POST" action="post.php" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="street" class="form-control" placeholder="Street Address" required></textarea>
                    </div>
                    <div class="form-group">
...
<div class="posts">
    ...
    $sql = "SELECT posts.*, users.fullname, users.profile_image, posts.province, posts.city, posts.barangay, posts.street, posts.status FROM posts
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<div style='display: flex; align-items: center;'>"; // Container for profile picture and username
        ...
        echo "<div style='display: flex; flex-direction: column; align-items: flex-end;'>"; // Container for "Mark as Sold" and 3-dot menu
    
        // Fetch the post status
        $status = $row['status'];
    
        // If the post belongs to the logged-in user, show the 3-dot menu and status toggle
        if ($row['username'] == $user) {
            // Status Toggle Button
            echo "<form method='POST' action='update_status.php' class='status-form' style='margin-bottom: 5px;'>
                    <input type='hidden' name='post_id' value='" . $row['id'] . "'>
                    <button type='submit' name='status' value='" . ($status == 'Available' ? 'Sold' : 'Available') . "' class='status-btn'>
        ...
        echo "</div>"; // Close action container
        ...
        // Post Status
        echo "<p class='status'>Status: <span class='status-badge " . strtolower($status) . "'>" . htmlspecialchars($status) . "</span></p>";
    
        // Post Description
        echo "<p class='description'>" . htmlspecialchars($row['description']) . "</p>";
        
        // Post Location
        echo "<p class='location'>Location: " . htmlspecialchars($row['province']) . ", " . htmlspecialchars($row['city']) . ", " . htmlspecialchars($row['barangay']) . ", " . htmlspecialchars($row['street']) . "</p>";
